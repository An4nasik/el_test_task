...
async def save_message(session: AsyncSession, user_id: str, role: str, content: str) -> Message:
    ...
    message = Message(conversation_id=conversation.id, role=role, content=content, created_at=datetime.now(timezone.utc).replace(tzinfo=None))
    session.add(message)
...
async def _get_or_create_conversation(session: AsyncSession, user_id: str) -> Conversation:
    ...
    if conversation:
        return conversation
    conversation = Conversation(user_id=user_id, created_at=datetime.now(timezone.utc).replace(tzinfo=None))
